...
Données : compétition kaggle "Grasp-and-Lift EEG Detection"

Ceci est un code modifié d'Alexandre Barachant (beat the benchmark). Ne me concentrant pas sur le prétraitement,
je suis parti de ce code pour appliquer le prétraitement sur les signaux et les sauvegarder au format .npz.

Note: les signaux d'un même sujet sont concatené et sont donc considérés comme un plus long signal.

Lire le fichier benchmark_preprocess_readme.txt pour voir un exemple ainsi qu'une explication pour charger les signaux.

Ci dessous se trouve donc le __doc__ d'Alexandre Barachant, laissé tel quel.
...
for subject in subjects:
    ...
    export_name = "Jupyter_notebook/train_filtered/filtered_"+str(subject)
    np.savez_compressed(export_name, feattr, labels)
    print("Done")
    
    withTest = False	#Passer à True si on veut exporter aussi les signaux d'entrainement
    ...
    if withTest:
        ...
        print("Sauvegarde du signal test...")
        export_name = "test_filtered/filtered_"+str(subject)
        np.savez_compressed(export_name, featte, ids)
        print("Done")
